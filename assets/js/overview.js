...
async function run(){
  ...
  const holdings=aggregateHoldings(core.instruments,core.transactions);const active=new Set(holdings.map(x=>x.symbol));const mm=new Map(market.map(x=>[x.symbol,x]));
  ...
  const sectors=aggregateSectors(joined);const etfs=joined.filter(x=>String(x.asset_type||x.market.asset_type||'').toLowerCase()==='etf');const etfValue=etfs.reduce((s,x)=>s+x.quantity*num(x.market.close||x.avgCost),0);
  ...
  root.innerHTML=`
    <div class="hero modern-hero"><div><span class="eyebrow">Decision dashboard</span><h2>Your portfolio at a glance</h2><p>A visual summary of capital, risk concentration, technical breadth and developments that may require action.</p></div><a class="btn primary" href="pages/transactions.html">+ Add transaction</a></div>
