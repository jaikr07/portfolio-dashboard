...
async function run(){
  const [core,res]=await Promise.all([loadCore(),loadResults()]);const holdings=aggregateHoldings(core.instruments,core.transactions);const active=new Set(holdings.map(x=>x.symbol));const insMap=new Map(core.instruments.map(x=>[x.symbol,x]));const etfs=holdings.filter(x=>String(x.asset_type||'').toLowerCase()==='etf').map(x=>x.symbol);
  ...
  root.innerHTML=`
    <div class="hero modern-hero"><div><span class="eyebrow">Fundamental health</span><h2>Financial decision dashboard</h2><p>Company cards prioritize growth, margin direction, cash conversion and reinvestment intensity. Missing fields are explained instead of being shown as meaningless dashes.</p></div></div>
